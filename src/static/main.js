$(document).ready(function () {
    ...
    $("#analysisForm").on("submit", async function (e) {
        ...
        try {
            const response = await axios.post("/api/plagiarism_web", { text_input: textInput });
            ...
            if (data.status === "success") {
                ...
                // ✅ Safely handle similarity_score
                let scoreValue = 0;
                if (result.similarity_score !== undefined && result.similarity_score !== null) {
                    if (typeof result.similarity_score === "string") {
                        scoreValue = parseFloat(result.similarity_score.replace('%', '')) || 0;
                    } else {
                        scoreValue = Number(result.similarity_score) || 0;
                    }
                }
                ...
                const ui = `
                    <div class="mb-3">
                        ...
                        <div class="progress" style="height: 25px;">
                            <div class="progress-bar ${badgeClass}" style="width: ${scoreValue}%;">${result.similarity_score || "0%"}</div>
                        </div>
                    </div>

                    <div class="mt-4">
                        <h5 class="fw-bold"><i class="bi bi-graph-up-arrow"></i> Interpretation</h5>
                        <span class="badge ${badgeClass} fs-6 p-2">${result.interpretation || "N/A"}</span>
                    </div>
                    ...
                    <div class="row text-center">
                        <div class="col-md-6">
                            <div class="card border-success shadow-sm mb-3">
                                ...
                                <div class="card-body">
                                    <p><strong>Total Tokens:</strong> ${result.lexical_analysis?.user?.total_tokens || 0}</p>
                                    <p><strong>Unique Tokens:</strong> ${result.lexical_analysis?.user?.unique_tokens || 0}</p>
                                    <p><strong>Keyword Count:</strong> ${result.lexical_analysis?.user?.keyword_count || 0}</p>
                        ...
                        <div class="col-md-6">
                            <div class="card border-primary shadow-sm mb-3">
                                ...
                                <div class="card-body">
                                    <p><strong>Total Tokens:</strong> ${result.lexical_analysis?.reference?.total_tokens || 0}</p>
                                    <p><strong>Unique Tokens:</strong> ${result.lexical_analysis?.reference?.unique_tokens || 0}</p>
                                    <p><strong>Keyword Count:</strong> ${result.lexical_analysis?.reference?.keyword_count || 0}</p>
                ...
                resultOutput.html(`<div class="alert alert-danger">❌ Error: ${data.message || "Unknown error"}</div>`);
            }
